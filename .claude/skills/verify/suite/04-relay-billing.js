...
(async () => {
  ...
  step('mirror (not the wt_team item) is what Capacity counts — planned has exactly one 0.5h entry',
    r.planned.filter(i => i.hours === 0.5).length === 1 && !r.planned.some(i => i.hours === 5 || i.hours === 4),
    r.planned.map(i => ({ name: i.name, h: i.hours })));
