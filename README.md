...
| `wt_boards` | Whiteboards, one per sub-code plus the project's 💭 Loose thoughts board: `{ id, projKey, scId, createdAt }`; meeting boards add `meetingTaskId` (one board per meeting-priority task, opened from the 📅 Meetings list) |
...
16. **Boards are a thinking surface, not a planner** (Sep 2026, Phase 0 of [`docs/vision-2026-09-boards.md`](docs/vision-2026-09-boards.md)). Stickies never carry hours or dates and are invisible to Capacity, the Timesheet and Allocations — a thought becomes work only by becoming a task. Capture is one field + Enter; the board strip never hides a board; the ⊞ Sort 2×2 view keeps unsorted stickies in a visible tray; quadrant labels are verbs (Do now / Up next / Steady / Park), with 👥 Delegate as a separate hand-off bin. The save-state chip on every board (synced / saved on this device / failed) is deliberate friction after the Aug 2026 data-loss incident. Board view prefs (`wt_proj_view` — dash / boards / list, `wt_board_open`, `wt_board_view`, `wt_board_panel`) are device-local. **Dash and command-panel tiles are read-only signals** (`_projSignals`): burn uses the same helpers as the Allocations Reconcile view, "my next deadline" counts only work that is yours, attention order is overdue > over budget > blocked > to delegate > waiting > inbox, and quiet programs compress to chips instead of hiding. **The timeline is a read-only drawing of existing dates**: bar start = `start`, else `createdAt`, else the deadline alone (legacy rows never get a guessed bar); relay first stages with no known start are derived from the estimate and drawn dashed; `dependsOn` only pulls a bar's start to its predecessor's due date; undated and held items keep their rows as chips. **A sticky becomes work only by promotion** (⋯ → Make this a task, or ✂ Task from selection on a meeting card): the task lands in the Inbox with no date or estimate, priority seeded from its 2×2 box (a sticky from the Delegate bin opens the Team section first), and the card becomes a **live linked card** that never copies fields — a deleted item leaves a dashed card you can unlink back into a sticky. 📌 pins existing items to their board (no duplicates); headings roll up the linked cards stacked beneath them spatially; tasks remember their sticky (💭). **Meetings are doorways**: every meeting-priority task on a program is listed beside the sub-code tiles and opens its own board (notes to bring, notes taken); past meetings fold behind "Past (n)". **A board is also the docket**: every open item of yours on that code appears as a dashed live card — drag one onto the board to place it; in ⊞ Sort 2×2 linked cards sit in the box their priority maps to and dragging between boxes changes the priority (urgent/high/med/low ↔ Do now/Up next/Steady/Park); 💭 Ideas holds unsorted stickies and 👥 Delegate is a hand-off bin: drop a task to pick who takes it, a sticky to flag it, and what others hold on that code is listed there. ↗ Assign and ◖ Baton (pass / finish / send back) are on cards, ☰ List rows and ▬ Timeline rows.
...
| Meeting boards | `_projMeetings`, `boardOpenMeeting`, `boardGoMeeting` |
| Docket / priority by quadrant / assign & baton from anywhere | `_boardDocket`, `_boardMaterialize`, `_boardSetItemPriority`, `showBatonMenu`, `_assignBtnFor` |
...
| ⌂ Dash / command panel / program signals | `renderProjDash`, `_projSignals`, `_scTileHtml`, `boardTogglePanel` |
| ▬ Timeline | `renderProjTimeline`, `_tlItems`, `_tlRowHtml`, `editStart` / `editDependsOn` (task form 📅 section) |
